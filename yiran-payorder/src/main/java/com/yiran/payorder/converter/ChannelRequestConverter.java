...
public class ChannelRequestConverter implements BasicConstant {
	 ...
	 * @return 渠道资金请求对象
	 */
	public static ChannelFundRequest covert(PayInstOrder instOrder, ICombineCallbackService callbackService,
		...
		case DEBIT:
			...
			case TRUSTCOLLECT:
				...
				if (StringUtils.isNotBlank(instOrder.getExtension().get(ExtensionKey.STORE_ID.key))) {
					request.getExtension().put(ExtensionKey.STORE_ID.key,
							instOrder.getExtension().get(ExtensionKey.STORE_ID.key));
				}

				if (StringUtils.isNotBlank(instOrder.getExtension().get(ExtensionKey.DISABLE_PAY_CHANNELS.key))) {
		...
		case MANUAL_REFUND:// TODO 需结算后台统一使用接口实现
			...
			for (ChannelRequestExtensionMapping mapping : ChannelRequestExtensionMapping
				...
				PropertyValueUtil.setValue(request, mapping, instOrder.getExtension().get(mapping.getExtensionKey()));
			}
			break;
		case SINGLE_PAY:
	...
	private static void buildRefund(RefundRequest request, InstRefundOrder order) {
		...
		request.setOrignalOrderSettleTime(DateUtil.parse(order.getFundinDate()));
		request.setOrignalOrderSubmitTime(DateUtil.parse(order.getFundinDate()));
		if (FundChannelApiType.MANUAL_REFUND.equals(order.getFundChannelApi().getApiType())) {
			...
			if (StringUtils.isNotEmpty(ManualRefundType.replaceMessage(order.getMemo()))) {
				request.getExtension().put(ExtensionKey.MEMO.key, ManualRefundType.replaceMessage(order.getMemo()));
			} else {
	...
	private static void buildFundout(FundoutRequest request, InstFundoutOrder order) {
		...
		Boolean isSameBank = order.getInstCode().equalsIgnoreCase(order.getFundChannel().getInstCode());
	 ...
	 * @return 渠道请求特定扩展信息
	 */
	private static Map<String, String> filterExtKey(Map<String, String> orgiMap, String apiCode, String fundChannelCode,
			List<TmFundChannelApiParam> filterList, ITmChannelTransInfoService channelTransInfoService) {
		...
		for (Iterator<TmFundChannelApiParam> iterator = filterList.iterator(); iterator.hasNext();) {
			...
			if (apiParam.getEnable().equals(YesNo.YES.getCode()) && apiParam.getApiCode().equals(apiCode)
				...
				if (YesNo.YES.equals(apiParam.getIsChannelTrans())) {
					Map<String, String> channelTransInfo = channelTransInfoService.getTransInfo(fundChannelCode,
							apiParam.getTransCode());
	...
	private static void buildCommonReq(PayInstOrder instOrder, ChannelRequest request,
		...
		if (callbackService != null) {
			if (request.getApiType().equals(FundChannelApiType.SINGLE_REFUND)) {
				request.setCallbackServerUrl(callbackService.getCallBackUrl(instOrder.getFundChannel(), SERVER_URL,
						FundChannelApiType.REFUND_NOTIFY));
				request.setCallbackPageUrl(callbackService.getCallBackUrl(instOrder.getFundChannel(), PAGE_URL,
						FundChannelApiType.REFUND_NOTIFY));
			} else if (request.getApiType().equals(FundChannelApiType.DEBIT)) {
				request.setCallbackServerUrl(callbackService.getCallBackUrl(instOrder.getFundChannel(), SERVER_URL,
						FundChannelApiType.NOTIFY));
				request.setCallbackPageUrl(callbackService.getCallBackUrl(instOrder.getFundChannel(), PAGE_URL,
						FundChannelApiType.NOTIFY));
			} else {
				request.setCallbackServerUrl(callbackService.getCallBackUrl(instOrder.getFundChannel(), SERVER_URL,
						FundChannelApiType.VERIFY_SIGN));
				request.setCallbackPageUrl(callbackService.getCallBackUrl(instOrder.getFundChannel(), PAGE_URL,
						FundChannelApiType.VERIFY_SIGN));
			}
			// end
	 ...
	 * @return
	 */
	public static QueryRequest covertQuery(PayInstOrder instOrder, ITmChannelTransInfoService channelTransInfoService) {
		...
		if (instOrder instanceof InstRefundOrder) {
			...
			request.setOriginalInstOrderSettleTime(DateUtil.parse(instRefundOrder.getFundinDate()));
			request.setOriginalInstOrderSubmitTime(DateUtil.parse(instRefundOrder.getFundinDate()));
		}
